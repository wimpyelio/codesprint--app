"""
User Stats Router - Provides personal analytics and progress dashboard.
Aggregates metrics: completion rate, total XP, badges earned, streaks.
"""
...
from app.models import User, UserProgress, Achievement, Project
...
def get_user_stats(
    ...
    Single query pattern with aggregations for efficiency.
    """
    ...
    completion_rate = (len(completed_projects) / total_projects * 100) if total_projects > 0 else 0
    ...
    xp_per_level = 1000
    xp_this_level = current_xp % xp_per_level
    xp_to_next = xp_per_level - xp_this_level
    ...
    # Streak calculation (placeholder - needs daily tracking table)
    last_completed = max(
        (p.completed_at for p in completed_projects if p.completed_at),
        default=None
    )
    
    current_streak = 1 if last_completed and (datetime.utcnow() - last_completed).days <= 1 else 0
...
def get_public_user_stats(
    ...
    """
    Public profile stats visible to other users.
    Returns limited info: XP, level, completion count, badges.
    """
...
def get_progress_by_difficulty(
    ...
    Useful for identifying which difficulty levels user excels at.
    """
    ...
    for p in progress:
        ...
        if project:
            ...
            by_difficulty[difficulty]["total_xp"] += p.xp_earned or 0
...
def get_user_badges(
    ...
    """
    Get all achievements/badges earned by current user.
    """
    badges = db.query(Achievement).filter(
        Achievement.user_id == current_user.id
    ...
    return [
        {
            ...
            "description": BADGE_DESCRIPTIONS.get(b.badge_name, "Achievement unlocked!")
        }
        for b in badges
    ]

# Badge definitions - would be moved to database in production
BADGE_DESCRIPTIONS = {
