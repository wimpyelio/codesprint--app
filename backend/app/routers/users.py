...
from app.models import Achievement, Project, User
...
) -> List[schemas.AchievementResponse]:
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def add_achievement(
    user_id: int,
    achievement_payload: schemas.AchievementCreate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    ...
    achievement = Achievement(
        user_id=user_id,
        badge_name=achievement_payload.badge_name,
        description=achievement_payload.description,
        badge_icon=achievement_payload.badge_icon,
    )
    db.add(achievement)
    db.commit()
