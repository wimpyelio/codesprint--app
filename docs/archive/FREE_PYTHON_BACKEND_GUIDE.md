...
- ✅ Login/register is fully implemented and tested.
- ✅ API now uses `/api/auth/register` and `/api/auth/login`.
- ✅ Auth now includes bcrypt password hashing and JWT token issuance.
- ✅ Backend rejects disposable/burner emails (example blocked: `10minutemail.com`, `temp-mail.org`).
- ✅ Frontend `API_BASE_URL` in `src/services/api.js` set to `http://localhost:8004/api`.
- ✅ Docs updated to reflect actual runtime port `8004`.

### Quick Status

- **Auth readiness**: YES, login/register is ready.
- **DB setup**: Completed with users/projects/test_cases/achievements tables.
- **Next step**: persist user progress to backend (project save/XP update).
    limit: int = 100,
    tier: str = None,
    db: Session = Depends(get_db)
):
